...
| --- | --- |
| **/collection:** *CollectionURL* | Specifies the URI of the project collection or Azure DevOps Services (Azure DevOps Services) account. For example: </br> **On-premises TFS format:** ```http://ServerName:Port/VirtualDirectoryName/CollectionName``` </br> If no virtual directory is used, then the format for the URI is the following: ```http://ServerName:Port/CollectionName```. </br> **Azure DevOps Services format:** http://AccountName.visualstudio.com.DefaultCollection |
