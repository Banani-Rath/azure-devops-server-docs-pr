---
title: How to install Azure DevOps Server Proxy or Team Foundation Proxy
...
## Prerequisites: 
To follow this procedure, you must be a member of the **Administrators** security group on the server on which you are configuring Team Foundation Server Proxy. You must be a member of the Project Collection Administrators group on TFS. 
...
0. On the **Tools** menu, choose **Options**.

0. In the **Options** dialog box, expand **Source Control**, and then choose **Visual Studio Team Foundation Server**.

0. Select the **Use proxy server for file downloads** check box.

0. In the **Proxy server name box**, type the name of the server running Team Foundation Server Proxy.

0. In the **Port** box, type the listener port for Team Foundation Server Proxy. By default, Team Foundation Server Proxy listens for client requests on port 8081.
