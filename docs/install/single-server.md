...
0. Choose **Configure Azure DevOps Server** and then choose **Start Wizard**. 

	![Start Wizard](_img/install-single-server/configuration-center-start-wizard.png) 

0. **Basic**. Choose the **New Deployment - Basic** option and then choose **Next**. 

	![New deployment, Basic page](_img/install-single-server/basic-3.png) 

0. **Language**. Select the **Language** for your deployment and choose **Next**.

	![Basic>Language](_img/install-single-server/basic-language-2.png) 

0. **SQL Server Instance**. Select the SQL Server instance you want to use. For evaluation purposes, choose **Install SQL Server Express**. For production purposes, choose **Use an existing SQL Server Instance**. Then, choose **Next**.
...
0. **Application Tier**. Choose the web site settings you want to use, which includes whether to use HTTP or HTTPS bindings, and then choose **Next**. For more information, see [Web site settings](../admin/websitesettings.md).

	![Basic>Application Tier](_img/install-single-server/basic-app-tier-5.png) 

0. **Search**. Select whether to install and configure **Search** features or use an existing search service. Installing and configuring **Search** supports Code, Work Item and Wiki search features. For more information, see [Configure search](/azure/devops/project/search/administration#config-tfs). Choose **Next**.

	![Basic>Search](_img/install-single-server/basic-search-8.png) 

	<!--- QUESTION: Need to provide hints on what to input re credentials --> 
0. **Review**. Review your selections and choose **Verify**, the wizard runs readiness checks to validate your environment and your setting selections. If it's successful, you can configure your deployment. Otherwise, fix any errors and rerun the readiness checks.

0. **Readiness Checks**. When all checks have passed, you're ready to complete the configuration. Choose **Configure**.
...
0. **Configure**. The wizard starts configuring each feature and displays the progress. When complete, choose **Next**.

	![Basic>Configure](_img/install-single-server/basic-configure-success-9.png) 

0. **Complete**. Review your configuration results and choose **Copy Full Path** to copy the link to the configuration log file. Choose the Azure DevOps Server link to open the web portal. 
...
0. **Advanced**. Choose the **New Deployment - Advanced** option and then choose **Next**. 
...
0. **Database**. Specify the SQL Server instance you have set up for use with Azure DevOps Server. Then, choose **Next**.

	![Advanced>Database](_img/advanced/advanced-db-5.png) 

0. **Account**. Specify the service account to use. 
...
0. **Search**. Select whether to install and configure **Search** features or use an existing search service. Installing and configuring **Search** supports Code, Work Item and Wiki search features. For more information, see [Configure search](/azure/devops/project/search/administration#config-tfs). Choose **Next**.
	...
	<!--- QUESTION: Need to provide hints on what to input re credentials --> 
<a id="reporting" /> 
0. **Reporting**. Uncheck the **Configure Reporting** checkbox if you don't plan on using SQL Server Reporting Services or haven't yet installed SQL Server Analysis Services and SQL Server Reporting Services. Otherwise, keep the box checked and choose **Next**.

	![Advanced>Reporting](_img/advanced/advanced-reporting-9a.png) 

0. **Reporting Services**. Enter the name of the server where you installed SQL Server Reporting Services. Then, choose **Populate URLs**. Choose **Next**.

	![Advanced>Reporting Services](_img/advanced/advanced-reporting-9b.png) 

0. **Analysis Services**. Enter the name of the server where you installed SQL Server Reporting Services, and then choose **Test**. Choose **Next**.

	![Advanced>Analysis Services](_img/advanced/advanced-reporting-9c.png) 

0. **Report Reader Account**. Enter the credentials for the service  account you've set up for reporting, and then choose **Test**. Choose **Next**. 

	![Advanced>Report Reader Account](_img/advanced/advanced-reporting-9d.png)  

0. **Project Collection**. Leave the **Create a new team project collection** checkbox checked. You must have at least one project collection in which to define projects. Leave the project name as is, *DefaultCollection*, or give it a new name and optionally a description.  
...
0. **Review**. Review the configuration settings you've made and choose **Verify**.  

	![Advanced>Review](_img/advanced/advanced-review-verify-11.png)  
0. **Readiness Checks**. The system performs a final pass to ensure the configuration settings are valid. When all checks have passed, you're ready to complete the configuration. Choose **Configure**.
...
0.	When your project has been created, the project summary page appears. To learn more, see [Share your project mission, view project activity](/azure/devops/project/wiki/project-vision-status).
